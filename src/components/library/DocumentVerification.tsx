...
export const DocumentVerification = ({
  ...
  const verifyDocument = async () => {
    ...
    try {
      ...
      const currentHash = Array.from(hashArray).map(b => b.toString(16).padStart(2, '0')).join('');
      ...
      recordDocumentEvent(status === 'verified' ? 'DOCUMENT_VERIFIED' : 'DOCUMENT_MODIFIED', documentId, documentName, 'current-user',
      // In a real app, get this from authentication context
      userEmail || 'user@example.com',
      // Now using actual user email
      ...
      toast[status === 'verified' ? 'success' : 'error'](status === 'verified' ? "Document verified successfully" : "Document verification failed", {
        description: status === 'verified' ? "The document integrity is verified and matches our records." : "The document may have been modified since it was last stored."
      });
  ...
  return <Card className="w-full shadow-sm">
      ...
      <CardContent className={`space-y-4 ${isMobile ? "px-4" : ""}`}>
        <div className="grid gap-4">
          ...
          <div>
            <Input id="document-verification" type="file" onChange={handleFileChange} className="mb-3 sm:mb-4 text-sm" />
            {file && <p className={`text-xs sm:text-sm bg-muted p-2 rounded ${isMobile ? "break-all" : ""}`}>
                Selected: {file.name} ({(file.size / 1024).toFixed(2)} KB)
              </p>}
          </div>
          
          <Button onClick={verifyDocument} disabled={!file} className="w-full text-sm" size={isMobile ? "sm" : "default"}>
            Verify Document Integrity
          </Button>
        </div>

        {verificationResult.status && <div className={`mt-4 sm:mt-6 p-3 sm:p-4 rounded-md border ${verificationResult.status === 'verified' ? 'bg-green-50 border-green-200 dark:bg-green-950/20 dark:border-green-900' : 'bg-red-50 border-red-200 dark:bg-red-950/20 dark:border-red-900'}`}>
            <div className="flex items-center mb-3 sm:mb-4">
              {verificationResult.status === 'verified' ? <ShieldCheck className="h-6 w-6 sm:h-8 sm:w-8 text-green-600 dark:text-green-500 mr-2" /> : <ShieldX className="h-6 w-6 sm:h-8 sm:w-8 text-red-600 dark:text-red-500 mr-2" />}
              <h3 className="text-base sm:text-lg font-medium">
                {verificationResult.status === 'verified' ? 'Document integrity verified' : 'Document may be altered!'}
              </h3>
            </div>
      ...
      <CardFooter className={`${isMobile ? "px-4 py-4 flex-col" : ""}`}>
        {verificationResult.status && <Button onClick={viewAuditTrail} variant="outline" size={isMobile ? "sm" : "default"} className={`text-sm ${isMobile ? "w-full mb-2" : ""}`}>
